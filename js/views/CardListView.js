	...
	enter_word: function (e) {
		...
		if (input.length > 2) {
			$.each(me.autocomplete, function (i, v) {
				if (v.toLowerCase().match(input.toLowerCase())) complete.push(v);
			});

			if (complete.length > 0) $("#autocomplete").html("<ul></ul>");
