var Card = Backbone.View.extend({
	...
	add_point: function (e) {
		...
		$(".card-body > nav").html(
			'<button type="button" class="btn btn-info" id="get-other" data-next="true">Выбрать следующую</button>'
		);
	...
	interval: function (i) {
		...
		if (i > time) {
			try {
				if (App.settings.sound) $("#beep")[0].play();
			} catch (e) {
			...
			$.get("js/templates/" + me.next + ".html", function (template) {
				...
				$(".card-body > nav").append(
					'<div class="no-one"><button type="button" id="get-other" class="btn btn-default btn-xs non-command" data-next="true">Выбрать другую карточку</button></div>'
				);
			});
		}
		if (i == limit)
	...
	get_card: function (btn) {
		if (!activity_words && activity_words.length == 0) {
			console.error("[E] Activity Words data is missing! Check db/cards.json file.");
			return;
		}
